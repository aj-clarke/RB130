...
Rules:
  ...
  - Takes an array as arg; block as part of method invocation
  - Return new array of values returned from each block (yield)
...
Iterate each with object (new_arr = new array|(ele, new_arr|
  new_arr << yield(ele)
end
-----------------------
Code:
...
def map(arr)
  arr.each_with_object(new_arr = []) { |ele| new_arr << yield(ele) }
end
